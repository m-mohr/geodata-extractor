...
public class OSRef extends CoordinateSystem {
  ...
  public OSRef(String zone, Integer east, Integer north) throws IllegalArgumentException {

    super(OSGB36Datum.getInstance());

    char char1 = zone.charAt(0);
    char char2 = zone.charAt(1);
	
	Long correctionEast = Math.round(Math.pow(10d, 5 - east.toString().length()));
    east = east * correctionEast.intValue();
	Long correctionNorth = Math.round(Math.pow(10d, 5 - north.toString().length()));
    north = north * correctionNorth.intValue();
