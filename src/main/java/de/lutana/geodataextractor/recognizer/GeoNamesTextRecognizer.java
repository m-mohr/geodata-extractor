...
import de.lutana.geodataextractor.recognizer.nlp.Demonyms;
...
public class GeoNamesTextRecognizer implements TextRecognizer {
	...
	public GeoNamesTextRecognizer(LuceneIndex index) throws IOException, ClassNotFoundException {
		this.index = index;
	...
	public boolean recognize(String text, LocationCollection locations, double weight) {
		...
		for (GeoName geoname : bestCandidates) {
			...
			if (l != null) {
				...
				l.setWeight(weight);
				LoggerFactory.getLogger(getClass()).debug("Parsed location " + l + " from GeoNamesTextRecognizer.");
				locations.add(l);
			}
			i++;
		}
