...
/**
 * Just a dumb country recognizer for the captions.
 * 
 * Using this to test the framework until we have something useful implemented.
 * 
 * @author Matthias Mohr
 */
public class DumbCountryTextRecognizer implements TextRecognizer {
	
	private final HashMap<String, Location> countryMap;
	
	public DumbCountryTextRecognizer() {
	...
	public boolean recognize(String text, LocationCollection locations, double weight) {
		...
		while(it.hasNext()) {
			...
			if (text.contains(entry.getKey().toLowerCase())) {
				...
				LoggerFactory.getLogger(getClass()).debug("Parsed location " + l + " from DumbCountryRecognizer.");
				locations.add(l);
				return true;
			}
		}
