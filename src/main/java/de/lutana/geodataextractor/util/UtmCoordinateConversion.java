...
public class UtmCoordinateConversion {
	...
	private class UTM2LatLon {
		...
		public double[] convertUTMToLatLong(String UTM) {
			Matcher m = GeoTools.UTM_PATTERN.matcher(UTM);
			if (!m.find()) {
				return null;
			}
			...
			String latZone = m.group(2);
			easting = Double.parseDouble(m.group(3));
			northing = Double.parseDouble(m.group(4));
			...
			latlon[0] = GeoTools.roundLatLon(latitude);
			latlon[1] = GeoTools.roundLatLon(longitude);
			return latlon;
